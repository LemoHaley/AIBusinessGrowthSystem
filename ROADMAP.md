...
| 6    | 业务闭环      | 5    | 一次生成落三表记录；parent 越权被拦    | 部分完成 |
...
> 执行调整：AI/RAG 外部凭证未备，先落地"本地子集"（不依赖外部服务的 6.1/6.2/6.4 + ai 模块纯 DB 接口）；6.3 AI 报告闭环待阶段 5 完成后补齐。

**任务**：

- [x] tenant/user/class/student CRUD 接口（全部写操作 POST）
- [x] report 模块：列表/详情，parent 强制过滤 student.parentUserId（第 5.6 节）
- [ ] AI 报告闭环：扣积分 → AI 生成 → 事务写 point_ledger + ai_logs + ai_reports（第 6.4 节时序，待阶段 5）
- [x] dashboard/summary 聚合接口
- [x] ai 模块纯 DB 子集：agents 配置查询/更新（全局默认 + 租户自定义合并）+ logs 分页

**完成标准**：一次报告生成产出三表记录；家长 A 传家长 B 的 studentId 查不到数据。
（本地子集已验证：parent 越权接口 40003、报告行级过滤空列表、看板聚合与趋势；三表记录验证随 6.3）
**提交**：`269b2cc` feat: 阶段6' 业务模块本地子集（6.3 补齐后另成提交）
